...
function init()
  ...
  if g_game.isOnline() then
    show()
	--ProtocolGame.registerExtendedOpcode(126, function(protocol, opcode, buffer) setTVBLOCK(buffer) end)
  end
end
...
--[[function setTVBLOCK(buffer)
  ...
  unbindWalkKey('Down')
  unbindWalkKey('Left')
  unbindWalkKey('Numpad8')
  unbindWalkKey('Numpad9')
  unbindWalkKey('Numpad6')
  unbindWalkKey('Numpad3')
  unbindWalkKey('Numpad2')
  unbindWalkKey('Numpad1')
  unbindWalkKey('Numpad4')
  unbindWalkKey('Numpad7')
	else 
	   TVBLOCK = false
	   bindKeys()
	end
...
function bindKeys()
  ...
  g_keyboard.bindKeyDown('Ctrl+Q', function() tryLogout(true) end, gameRootPanel)
  g_keyboard.bindKeyDown('Ctrl+L', function() tryLogout(true) end, gameRootPanel)
...
function processMouseAction(menuPosition, mouseButton, autoWalkPos, lookThing, useThing, creatureThing, attackCreature)
  ...
  --if TVBLOCK and not ((g_mouse.isPressed(MouseLeftButton) and mouseButton == MouseRightButton) or (g_mouse.isPressed(MouseRightButton) and mouseButton == MouseLeftButton)) then 
  --return false 
  --end
...
function moveStackableItem(item, toPos)
  ...
  g_keyboard.bindKeyPress("Up", function() check() spinbox:setValue(spinbox:getValue()+10) end, spinbox)
  g_keyboard.bindKeyPress("Down", function() check() spinbox:setValue(spinbox:getValue()-10) end, spinbox)
